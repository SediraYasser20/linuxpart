 ...
 * Copyright (C) 2024       FrÃƒÂ©dÃƒÂ©ric France         <frederic.france@free.fr>
 *
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		if ($totalqty > 0) {  // There is at least one thing to ship
			for ($i = 1; $i <= $num; $i++) {
				...
				// Get the quantity for the current input line
				$current_input_qty = GETPOSTFLOAT($qty);
				// Get the original ordered quantity from the source order line
				$ordered_qty = $lineToTest->qty;

				// Fetch total quantity already received for this specific order line from other validated receptions
				$total_already_received_qty = 0;
				$sql_already_received = "SELECT SUM(rd.qty) as total_received";
				$sql_already_received .= " FROM ".MAIN_DB_PREFIX."receptiondet_batch as rd";
				$sql_already_received .= " INNER JOIN ".MAIN_DB_PREFIX."reception as r ON rd.fk_reception = r.rowid";
				$sql_already_received .= " WHERE rd.fk_elementdet = ".$lineToTest->id;
				$sql_already_received .= " AND r.fk_statut IN (".Reception::STATUS_VALIDATED.", ".Reception::STATUS_CLOSED.")"; // Only count validated or closed receptions

				$resql_already_received = $db->query($sql_already_received);
				if ($resql_already_received) {
					if ($db->num_rows($resql_already_received) > 0) {
						$obj_received = $db->fetch_object($resql_already_received);
						if ($obj_received && !is_null($obj_received->total_received)) {
							$total_already_received_qty = $obj_received->total_received;
						}
					}
					$db->free($resql_already_received);
				} else {
					dol_print_error($db);
					$error++; // Consider this a critical error
					continue;
				}

				$remaining_allowed_qty = $ordered_qty - $total_already_received_qty;

				// Check if current input quantity exceeds remaining allowed quantity
				if ($current_input_qty > $remaining_allowed_qty) {
					// Try to get a meaningful product description
					$product_description = $langs->trans("UnknownProduct");
					if (!empty($lineToTest->fk_product)) {
						$tmpproduct = new Product($db);
						$tmpproduct->fetch($lineToTest->fk_product);
						$product_description = $tmpproduct->getNomUrl(0) . ' - ' . ($tmpproduct->label ? $tmpproduct->label : $tmpproduct->ref);
					} elseif (!empty($lineToTest->label)) {
						$product_description = $lineToTest->label;
					} elseif (!empty($lineToTest->description)) {
						$product_description = dol_trunc($lineToTest->description, 20);
					}

					setEventMessages($langs->trans("QtyReceivedExceedsRemainingAllowedForProduct", dol_escape_htmltag($product_description), $current_input_qty, $remaining_allowed_qty, $ordered_qty), null, 'errors');
					$error++;
					continue; // Skip adding this line and proceed to the next iteration
				}
				...
				if ($current_input_qty > 0 || ($current_input_qty == 0 && getDolGlobalString('RECEPTION_GETS_ALL_ORDER_PRODUCTS'))) {
					// If RECEPTION_GETS_ALL_ORDER_PRODUCTS is enabled and input is 0, set qty to remaining allowed
					if ($current_input_qty == 0 && getDolGlobalString('RECEPTION_GETS_ALL_ORDER_PRODUCTS')) {
						$current_input_qty = $remaining_allowed_qty;
						if ($current_input_qty <= 0) { // If nothing remaining, skip
							continue;
						}
					}
...
if (!array_key_exists($line->id, $arrayofpurchaselinealreadyoutput)) {
    print $quantityDelivered;
}

print '<input name="qtydelivered'.$indiceAsked.'" id="qtydelivered'.$indiceAsked.'" type="hidden" value="'.dol_escape_htmltag($quantityDelivered).'">';
print '</td>';
 ...
 {
						print '<td>';
						$readonly = $user->admin ? '' : 'readonly';
print '<input class="width75 right" name="cost_price'.$indiceAsked.'" id="cost_price'.$indiceAsked.'" value="'.$cost_price.'" '.$readonly.'>';
				...
				if (!empty($extrafields)) {
					$colspan = 5; // Base colspan: Description, Comment, QtyOrdered, QtyReceived, QtyToReceive
					if (isModEnabled('stock')) { // Warehouse column
						// $colspan++; // This seems to be implicitly handled by the base 5, as warehouse is always shown if stock module on.
									   // No, the base 5 is: Description, Comment, QtyOrdered, QtyReceived, QtyToReceive.
									   // Let's re-evaluate base and additions.
									   // Base: Desc, Comment, QtyOrd, QtyRec, QtyToRec = 5
									   // If stock enabled: +1 (Warehouse) = 6
									   // If productbatch: +1 (batch_number) = (+1 more if stock was already on, so 7)
									   // If !PRODUCT_DISABLE_SELLBY: +1 (SellByDate)
									   // If !PRODUCT_DISABLE_EATBY: +1 (EatByDate)
									   // If admin and buying price shown: +1
					}

					// Recalculating colspan based on visible columns in the main product line section:
					// Minimum columns: Description, Comment, QtyOrdered, QtyReceived, QtyToReceive = 5
					$colspan = 5;
					if (getDolGlobalInt('STOCK_CALCULATE_ON_RECEPTION') || getDolGlobalInt('STOCK_CALCULATE_ON_RECEPTION_CLOSE')) {
						if ($user->admin) { // Buying price column is shown for admin
							$colspan++;
						}
					}
					if (isModEnabled('stock')) { // Warehouse column
					...
					if (isModEnabled('productbatch')) { // batch_number column
						$colspan++;
						if (!getDolGlobalInt('PRODUCT_DISABLE_SELLBY')) { // SellByDate column
							$colspan++;
						}
						if (!getDolGlobalInt('PRODUCT_DISABLE_EATBY')) { // EatByDate column
							$colspan++;
						}
					}
