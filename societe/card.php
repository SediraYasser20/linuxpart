...
$help_url = 'EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas|DE:Modul_GeschÃ¤ftspartner';
	...
	if ($action == 'create') {
		...
		if (empty($reshook)) {
			...
			*/
			...
			print '<td>';
			print '<input id="assujtva_value" name="assujtva_value" type="checkbox" ' . (GETPOSTISSET('assujtva_value') ? (GETPOST('assujtva_value', 'alpha') != '' ? ' checked="checked"' : '') : '') . ' value="1">';
			print '</td>';
			if ($conf->browser->layout == 'phone') {
				print '</tr><tr>';
			}
  ...
  $.ajax({
	...
	success: function(data) {
		...
		if (Array.isArray(firmActUser)) {
			var primaryFirmAct = firmActUser.find(item => item.firmActKindDescr === "ÎšÎ¥Î¡Î™Î‘"); // Find primary client activity
			if (primaryFirmAct) {
