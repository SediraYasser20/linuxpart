 ...
 * Copyright (C) 2012-2015  RaphaÃ«l Doursenaud      <rdoursenaud@gpcsolutions.fr>
		...
		if (getDolGlobalString('MAIN_USE_JQUERY_JEDITABLE') && !preg_match('/^select;|day|datepicker|dayhour|datehourpicker/', $typeofdata)) { // TODO add jquery timepicker and support select
			...
			if ($editmode) {	// edit mode
				...
				if (preg_match('/^(string|safehtmlstring|email|phone|url)/', $typeofdata)) {
					...
					// So we convert & into &amp; so a string like 'a &lt; <b>b</b><br>Ã©<br>&lt;script&gt;alert('X');&lt;script&gt;' stay a correct html and is not converted by textarea component when wysiwyg is off.
...
if ($stocktag == 1) {
    $opt .= ' class="product_line_stock_ok" data-html="'.dol_escape_htmltag($outref.' - '.dol_trunc($outlabel, $maxlengtharticle)).dolPrintHTMLForAttribute($labeltoshowhtmlstock).'"';
}
if ($stocktag == -1) {
    $opt .= ' class="product_line_stock_too_low" data-html="'.dol_escape_htmltag($outref.' - '.dol_trunc($outlabel, $maxlengtharticle)).dolPrintHTMLForAttribute($labeltoshowhtmlstock).'"';
}
		$opt .= '>';
	...
	public function select_produits_fournisseurs_list($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $filterkey = '', $statut = -1, $outputmode = 0, $limit = 100, $alsoproductwithnosupplierprice = 0, $morecss = '', $showstockinlist = 0, $placeholder = '')
	{
		...
		if ($result) {
			...
			while ($i < $num) {
				...
				//           ["label"]=>string(76) "ppp (<strong>f</strong>ff2) - ppp - 20,00 Euros/1unitÃ© (20,00 Euros/unitÃ©)"
				//      	 ["qty"]=>string(1) "1" ["discount"]=>string(1) "0" ["disabled"]=>bool(false)
				//}
				//var_dump($outval); var_dump(utf8_check($outval)); var_dump(json_encode($outval));
				//$outval=array('label'=>'ppp (<strong>f</strong>ff2) - ppp - 20,00 Euros/ UnitÃ© (20,00 Euros/unitÃ©)');
				//var_dump($outval); var_dump(utf8_check($outval)); var_dump(json_encode($outval));
	 ...
	 */
	public static function multiselectarray($htmlname, $array, $selected = array(), $key_in_label = 0, $value_as_key = 0, $morecss = '', $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1, $required = false)
	{
		global $conf, $langs;
		$out = '';
		...
		$out .= '<select id="' . $htmlname . '" class="multiselect' . ($useenhancedmultiselect ? ' multiselectononeline' : '') . ($morecss ? ' ' . $morecss : '') . '" multiple name="' . $htmlname . '[]"' . ($moreattrib ? ' ' . $moreattrib : '') . ($width ? ' style="width: ' . (preg_match('/%/', (string) $width) ? $width : $width . 'px') . '"' : '') . ($required ? ' required' : '') . '>' . "\n";
