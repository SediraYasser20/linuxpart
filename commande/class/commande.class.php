 ...
 * Copyright (C) 2021-2024  Fr̩d̩ric France     <frederic.france@free.fr>
	...
	*/
	...
	public function update_status_after_element_deletion($element_type)
	{
		...
		if ($element_type == 'shipment') {
			...
			$this->fetchObjectLinked(null, 'facture', $this->id, 'commande', 'source', 1); // Force refresh of linked objects
			dol_syslog("Order ref ".$this->ref.": update_status_after_element_deletion for invoice. Initial this->billed = " . $this->billed, LOG_DEBUG);
			...
			// After handling the billed flag, check if fk_statut needs adjustment.
			if ($this->billed == 0) { // Only consider changing fk_statut if the order is now not billed
				// Count total number of shipments linked to this order, regardless of their status.
				$sql_count_total_shipments = "SELECT COUNT(DISTINCT el.fk_target) as nb";
				$sql_count_total_shipments .= " FROM ".MAIN_DB_PREFIX."element_element as el";
				$sql_count_total_shipments .= " WHERE el.fk_source = ". (int) $this->id;
				$sql_count_total_shipments .= " AND el.sourcetype = '".$this->db->escape($this->element)."'";
				$sql_count_total_shipments .= " AND el.targettype = 'shipping'";

				$total_shipments = 0;
				$resql_count_total_shipments = $this->db->query($sql_count_total_shipments);
				if ($resql_count_total_shipments) {
					$obj_count_shipments = $this->db->fetch_object($resql_count_total_shipments);
					$total_shipments = $obj_count_shipments->nb;
				} else {
					$this->error = $this->db->lasterror();
					$this->db->rollback();
					return -4; // Error querying total shipments
				}

				// If there are no shipments at all, and the order was previously closed or shipment on process,
				// revert to validated status because the invoice that might have justified "billed" is gone, and no shipments affect status.
				if ($total_shipments == 0) {
					if ($this->status == self::STATUS_CLOSED || $this->status == self::STATUS_SHIPMENTONPROCESS) {
						dol_syslog("Order ref ".$this->ref.": Last invoice deleted AND no shipments exist at all. Reverting order status from ".$this->status." to ".self::STATUS_VALIDATED);
						$this->statut = self::STATUS_VALIDATED; // for internal consistency and triggers
						$this->status = self::STATUS_VALIDATED; // for internal consistency and triggers

						$sql_update_status = "UPDATE ".MAIN_DB_PREFIX.$this->table_element;
						$sql_update_status .= " SET fk_statut = ".self::STATUS_VALIDATED;
						// Also ensure date_cloture is nullified if we move away from CLOSED status due to this logic
						if ($initial_fk_statut == self::STATUS_CLOSED) {
							$sql_update_status .= ", date_cloture = NULL, fk_user_cloture = NULL";
						}
				...
				// If $total_shipments > 0, the fk_statut should remain as is (e.g., STATUS_CLOSED if shipments are delivered,
				// or STATUS_SHIPMENTONPROCESS if shipments are ongoing). The order is just not "Billed" anymore.
			}
		}

		// Determine if anything actually changed to set the $changed flag for the trigger
		// The status of $this->billed and $this->status might have been updated directly in this method or by classifyUnBilled
