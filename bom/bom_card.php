 ...
 * Copyright (C) 2019-2024  FrÃƒÂ©dÃƒÂ©ric France         <frederic.france@free.fr>
...
if (empty($reshook)) {
	...
	// Added by Jules: Force quantity to 1 for saving, before actions_addupdatedelete.inc.php processes it.
	// The 'add' action in the form becomes 'create' for the object, and 'update' action remains 'update'.
	// actions_addupdatedelete.inc.php handles these by populating $object from $_POST.
...
if ($action == 'create') {
	...
	// Added by Jules: Ensure BOM quantity is 1 and uneditable for display
	if (isset($object->fields['qty'])) {
		$object->fields['qty']['default'] = '1'; // Reinforce default for creation
		$object->fields['qty']['disabled'] = 1;  // Make it uneditable on the form
		$object->fields['qty']['noteditable'] = 1; // For templates that might use this over 'disabled'
	}
	$object->qty = 1; // Ensure the object instance itself has qty=1 for display purposes by commonfields_add
...
if (($id || $ref) && $action == 'edit') {
	...
	// $object->fetch() would have loaded the original qty. We override it here to ensure '1' is displayed.
	// The $_POST['qty'] = '1' modification earlier will handle the save.
	$object->qty = 1;
