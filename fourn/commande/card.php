...
// Check if the current user is in group 3 for the ReOpen button logic
$userIsInGroup3 = false;
if (!empty($user->id) && !empty($db)) { // Ensure $user and $db are available
    $sql_user_group = "SELECT COUNT(*) as user_count FROM ".MAIN_DB_PREFIX."usergroup_user WHERE fk_user = ".((int) $user->id)." AND fk_usergroup = 3";
    $resql_user_group = $db->query($sql_user_group);
    if ($resql_user_group) {
        $obj_user_group = $db->fetch_object($resql_user_group);
        if ($obj_user_group && $obj_user_group->user_count > 0) {
            $userIsInGroup3 = true;
        }
        $db->free($resql_user_group);
    } else {
        // Optional: Log error if query failed, dol_print_error($db);
    }
}

if (in_array($object->statut, array(3, 4, 5, 6, 7, 9))) { // Outer check for relevant statuses for Admin
    if ($user->admin) {
        // Admin sees button if status is one of 3, 4, 5, 6, 7, 9
        print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=reopen&token='.newToken().'">'.$langs->trans("ReOpen").'</a>';
    } elseif ($userIsInGroup3 && $object->statut == 3) {
        // Group 3 user sees button ONLY if status is 3 (Ordered)
        print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=reopen&token='.newToken().'">'.$langs->trans("ReOpen").'</a>';
    }
}
