 ...
 * Copyright (C) 2023-2024  FrÃƒÆ’Ã‚Â©dÃƒÆ’Ã‚Â©ric France     <frederic.france@free.fr>
...
public function createProduction(User $user, $notrigger = 0)
{
	...
	public function cancel($user, $notrigger = 0, $also_cancel_consumed_and_produced_lines = false, $skip_stock_movement = false)
	{
		...
		if ($skip_stock_movement) {
			...
			$result = $this->setStatusCommon($user, self::STATUS_CANCELLED_NO_STOCK_MOVEMENT, $notrigger, 'MRP_MO_CANCELLED_NO_STOCK_MOVEMENT');
			if ($result < 0) {
				$error++;
			}
		} else {
