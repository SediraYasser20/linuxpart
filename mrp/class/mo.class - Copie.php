...
public function create(User $user, $notrigger = 0)
{
    ...
    if (getDolGlobalString('PRODUIT_SOUSPRODUITS') && !getDolGlobalString('ALLOW_USE_KITS_INTO_BOM_AND_MO') && $this->fk_bom > 0) {
    ...
    // Handle BOM-related logic: set MO type and calculate manufacturing cost
    if ($this->fk_bom > 0) {
        include_once DOL_DOCUMENT_ROOT.'/bom/class/bom.class.php';
        $tmpbom = new BOM($this->db);
        $res = $tmpbom->fetch($this->fk_bom);
        if ($res > 0) {
            // Set MO type based on BOM
        ...
        // No BOM specified, set manufacturing cost to 0
        $this->manufacturing_cost = 0;
    }

    // Create the main MO record
    if (!$error) {
        $idcreated = $this->createCommon($user, $notrigger);
        if ($idcreated > 0) {
            // Explicitly update manufacturing_cost to ensure it's set
            $sql = "UPDATE ".MAIN_DB_PREFIX."mrp_mo SET manufacturing_cost = ".$this->db->escape($this->manufacturing_cost)." WHERE rowid = ".$idcreated;
    ...
    // Insert production lines from BOM
    if (!$error) {
        $result = $this->createProduction($user, $notrigger);
        if ($result <= 0) {
            $error++;
        }
    }
