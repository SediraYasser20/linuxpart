 ...
 * Copyright (C) 2012-2015  Marcos GarcÃ­a           <marcosgdf@gmail.com>
...
class Facture extends CommonInvoice
{
	...
	public function delete($user, $notrigger = 0, $idwarehouse = -1)
	{
		...
		// Ensure Commande class is available for later use
		...
		// Store linked orders before deleting links
		if ($this->id > 0) {
			// Use a direct query to get linked order IDs before links are removed.
			$sql_get_linked_orders = "SELECT fk_source FROM ".MAIN_DB_PREFIX."element_element";
			$sql_get_linked_orders .= " WHERE fk_target = ".((int) $this->id);
			$sql_get_linked_orders .= " AND targettype = 'facture'";
			$sql_get_linked_orders .= " AND sourcetype = 'commande'";
			$resql_orders = $this->db->query($sql_get_linked_orders);
			if ($resql_orders) {
				while ($obj_order = $this->db->fetch_object($resql_orders)) {
					$linked_order_ids[] = $obj_order->fk_source;
				}
				$linked_order_ids = array_unique($linked_order_ids); // Ensure unique IDs
				dol_syslog(get_class($this)."::delete - Found linked order IDs before link deletion: " . implode(', ', $linked_order_ids), LOG_DEBUG);
			} else {
				dol_syslog(get_class($this)."::delete - Error fetching linked order IDs: " . $this->db->lasterror(), LOG_ERR);
				// This is not necessarily a fatal error for invoice deletion itself.
			}
		}
		...
		if (!$error) {
			// Delete linked object (element_element entries)
			// This is where the link to the order is actually removed.
		...
		if (!$error) {
			...
			$resql = $this->db->query($sql); // Errors here are not made fatal for the main deletion

			// If invoice has consumed discounts
			// $this->fetch_lines(); // Avoid if it re-queries links that might be gone.
			$list_rowid_det = array();
			$sql_get_lines = "SELECT rowid FROM ".MAIN_DB_PREFIX."facturedet WHERE fk_facture = ".((int) $rowid);
			$resql_lines = $this->db->query($sql_get_lines);
			if ($resql_lines) {
				while($obj_line = $this->db->fetch_object($resql_lines)) {
					$list_rowid_det[] = $obj_line->rowid;
				}
			}
			...
			if (!$this->db->query($sql)) {
				...
				$this->errors[] = $this->error;
				$this->db->rollback();
				return -5; // Kept original error codes for consistency
			}
			...
			if (empty($this->lines) && ($this->type != self::TYPE_DEPOSIT && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_BILL') && $idwarehouse != -1)) {
				$this->fetch_lines(); // Ensure lines are loaded if not already
			}
			if ($this->type != self::TYPE_DEPOSIT && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_BILL') && $idwarehouse != -1) {
				...
				for ($i = 0; $i < $num; $i++) {
					if ($this->lines[$i]->fk_product > 0) {
						...
						// Errors in stock movement are logged but might not be fatal for invoice deletion itself.
						if ($result < 0) { dol_syslog(get_class($this)."::delete - Stock movement error: ".$mouvP->error, LOG_ERR); }
					}
				}
			}
			...
			if ($this->db->query($sqlef) && $this->db->query($sql) && $this->delete_linked_contact() >= 0) {
				...
				$resql = $this->db->query($sql);
				if ($resql) {
					// Delete record into ECM index
					$this->deleteEcmFiles(0);
					$this->deleteEcmFiles(1); // And physically
					...
					// If no error until here, we commit the invoice deletion.
					// Order status updates will happen after this commit.
					dol_syslog(get_class($this)."::delete transaction commit for invoice ID ".$this->id, LOG_DEBUG);
					$this->db->commit();

					// After successful deletion, update the source order's status
					if (!empty($linked_order_ids)) {
						// No need to check $error again as we committed.
						// Errors in order updates are logged but don't affect invoice deletion success.
						foreach ($linked_order_ids as $order_id) {
							$tmpOrder = new Commande($this->db);
							if ($tmpOrder->fetch($order_id) > 0) {
								...
								$update_result = $tmpOrder->update_status_after_element_deletion('invoice');
								if ($update_result < 0) {
									dol_syslog("Error updating status for order ID " . $tmpOrder->id . ". Error: " . $tmpOrder->error, LOG_ERR);
								}
							} else {
								dol_syslog("Failed to fetch order ID " . $order_id . " for status update.", LOG_WARNING);
							}
						}
					}
					return 1; // Invoice deletion successful
				} else {
				...
				$this->error = $this->db->lasterror()." sql=".$sql; // Error from facturedet or contact deletion
				$this->errors[] = $this->error;
				$this->db->rollback();
				return -4;
			}
		} else { // This else corresponds to if (!$error) before manipulating societe_remise_except etc.
			$this->db->rollback();
			return -2; // Error from earlier steps (trigger, deleteExtraFields, deleteObjectLinked)
		}
	}
	...
	public function setDraft($user, $idwarehouse = -1)
	{
		...
		if ($result) {
			...
			if ($error == 0) {
				...
				$this->fetchObjectLinked(null, 'commande', $this->id, 'facture');
				if (isset($this->linkedObjects['commande']) && is_array($this->linkedObjects['commande'])) {
					if (!class_exists('Commande')) {
						require_once DOL_DOCUMENT_ROOT . '/commande/class/commande.class.php';
					}
					foreach ($this->linkedObjects['commande'] as $order) {
						$tmporder = new Commande($this->db);
						$tmporder->fetch($order->id);
						$tmporder->classifyUnBilled($user);
					}
				}
