 ...
 * Copyright (C) 2015-2016	Marcos GarcÃ­a			<marcosgdf@gmail.com>
 * Copyright (C) 2015-2017	Alexandre Spangaro		<aspangaro@open-dsi.fr>
 * Copyright (C) 2016		Ferran Marcet   		<fmarcet@2byte.es>
 * Copyright (C) 2019		JC Prieto				<jcprieto@virtual20.com><prietojc@gmail.com>
 * Copyright (C) 2022-2024  FrÃ©dÃ©ric France         <frederic.france@free.fr>
...
class Account extends CommonObject
{
	...
	public function addline($date, $oper, $label, $amount, $num_chq, $categorie, User $user, $emetteur = '', $banque = '', $accountancycode = '', $datev = null, $num_releve = '', $amount_main_currency = null)
	{
		...
		// Defensive unsetting of properties that might have been dynamically added by hooks
		// and could interfere with AccountLine::insert() or its own call to insertExtraFields()
		// if they are misinterpreted as main table columns for llx_bank.
		if (isset($accline->category)) {
			unset($accline->category);
		}
		if (isset($accline->service)) {
			unset($accline->service);
		}
		if (isset($accline->resource)) {
			unset($accline->resource);
		}
		// Also unset 'user' just in case, as it was part of the original PaymentVarious object.
		// AccountLine does not have a 'user' property by default.
		if (isset($accline->user)) {
			unset($accline->user);
		}
