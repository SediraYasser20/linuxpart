...
$selected_user_id = GETPOSTINT('selected_user_id'); // Added for new user select
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $permissiontoadd) {
		...
		// TODO: Finalize which field to use for the selected user. Using fk_user_author temporarily.
		// A new field like fk_user_assignee (INT) or repurposing the existing 'user' (VARCHAR) field after clarification is needed.
		// If 'user' (VARCHAR) is used, we might store login or name. If a new FK, we store rowid.
		// For now, assuming we want to store the ID of the selected user.
		$object->selected_user_id = GETPOSTINT('selected_user_id'); // Will be mapped to a db field later
...
$help_url = 'EN:Module_Suppliers_Invoices|FR:Module_Fournisseurs_Factures|ES:MÃƒÂ³dulo_Facturas_de_proveedores|DE:Modul_Lieferantenrechnungen';
...
if ($action == 'create') {
	...
	$selecteduserid = GETPOSTINT('selected_user_id'); // Make sure it's available for pre-selection if form fails & reloads
	...
	// ($selected, $htmlname, $show_empty, $exclude=null, $disabled=0, $include='', $enableonly=array(), $force_entity='', $maxlength=0, $showstatus=0, $morefilter='', $show_every=0, $enableonlytext='', $morecss='', $notdisabled=0, $outputmode=0, $multiple=false, $forcecombo=0)
	print $form->select_dolusers(empty($object->selected_user_id) ? $selecteduserid : $object->selected_user_id, 'selected_user_id', 1, null, 0, '', array(), 0, 0, 0, '', 0, '', 'minwidth300');
	print '</td></tr>';
...
if ($id) {
	...
	if (!empty($object->selected_user_id) && $object->selected_user_id > 0) {
		// $object->selected_user_id is now populated by PaymentVarious::fetch()
		// based on the login stored in the 'user' column.
		$assignedUser = new User($db);
		if ($assignedUser->fetch($object->selected_user_id) > 0) {
			print $assignedUser->getNomUrl(1);
		} else {
			// User ID was stored, but user not found (e.g. deleted)
			// Or, selected_user_id was not populated correctly if login in 'user' column was invalid
			print $langs->trans("Unknown").' (ID: '.$object->selected_user_id.')';
			if (!empty($object->user)) {
				print ' - Login: '.dol_escape_htmltag($object->user);
			}
		}
	} elseif (!empty($object->user)) {
		// This case handles if 'user' column has a value but somehow selected_user_id wasn't populated
		// (e.g. if fetch logic for user by login failed but login exists)
		print dol_escape_htmltag($object->user).' ('.$langs->trans("LoginNotMatchedToUser").')';
	} else {
		// No user assigned or stored
		print $langs->trans("NotAssigned");
	}
	print '</td></tr>';
