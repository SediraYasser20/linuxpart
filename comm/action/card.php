...
// Action classify done
if (empty($reshook) && $action == 'classifydone') {
    if (GETPOST('token')) {
        $object->fetch($id);
        if ($object->userownerid == $user->id || $user->admin || (!empty($object->userassigned) && isset($object->userassigned[$user->id]))) {
            $db->begin();
            $object->percentage = 100;
            if (empty($object->datef) || $object->datef < dol_now()) {
...
if (empty($reshook)) {
    if ($action != 'edit') {
        // â€”â€”â€” Modify button logic â€”â€”â€”
        $showModifyButton = false;
        $isOwner = ($object->userownerid == $user->id);
        $isFinished = ($object->percentage >= 100 || $object->status == 9); // Adjust status check as needed

        // Show button if:
        // 1. Task is unfinished AND user is owner, OR
        // 2. User is admin (regardless of status)
        if (($isOwner && !$isFinished) || $user->admin) {
        ...
        // Classify Done button
        if ($object->percentage < 100 && ($object->userownerid != $user->id) && (!empty($object->userassigned) && isset($object->userassigned[$user->id]))) {
            print '<div class="inline-block divButAction"><a class="butAction" href="card.php?action=classifydone&id='.$object->id.'&token='.newToken().'">'.$langs->trans("ClassifyDone").'</a></div>';
        }

        // â€”â€”â€” To Clone (unchanged) â€”â€”â€”
