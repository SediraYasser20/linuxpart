 ...
 * Copyright (C) 2012-2013  C̩dric Salvador         <csalvador@gpcsolutions.fr>
 ...
 * Copyright (C) 2022       Vincent de Grandpr̩     <vincent@de-grandpre.quebec>
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $usercancreate) {
		...
		if ($type == Product::TYPE_PRODUCT && GETPOST('finished', 'alpha') == '-1') { // Nature of product is applicable only for products
			setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentities('NatureOfProduct')), null, 'errors');
			$action = "create";
			$error++;
		}
	...
	if ($action == 'update' && $usercancreate) {
		if (GETPOST('cancel', 'alpha')) {
			...
			if ($object->id > 0) {
				...
				if ($object->isProduct() && GETPOST('finished', 'alpha') == '-1') { // Nature of product is applicable only for products
					setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentities('NatureOfProduct')), null, 'errors');
					$error++;
				}
		...
		if (empty($reshook)) {
			...
			if ($type != 1) {	// Nature, Weight and volume only applies to products and not to services
				if (!getDolGlobalString('PRODUCT_DISABLE_NATURE')) {
					...
					print $formproduct->selectProductNature('finished', $object->finished, 0, 1, true);
					print '</td></tr>';
				}
			}
			...
			if (isModEnabled('category')) {
				...
				print img_picto('', 'category', 'class="pictofixedwidth"').$form->multiselectarray('categories', $cate_arbo, GETPOST('categories', 'array'), 0, 0, 'quatrevingtpercent widthcentpercentminusx', 0, 0, '', '', '', -1, true);
				print "</td></tr>";
			}
			...
			if (empty($reshook)) {
				...
				if ($object->isService()) {
					...
					if (!getDolGlobalString('PRODUCT_DISABLE_NATURE')) {
						// Nature
						print '<tr><td>'.$form->textwithpicto($langs->trans("NatureOfProductShort"), $langs->trans("NatureOfProductDesc")).'</td><td>';
						print $formproduct->selectProductNature('finished', (GETPOSTISSET('finished') ? GETPOST('finished') : $object->finished), 0, 1, true);
						print '</td></tr>';
					}
				}
				...
				if (isModEnabled('category')) {
					...
					print img_picto('', 'category', 'class="pictofixedwidth"').$form->multiselectarray('categories', $cate_arbo, $arrayselected, 0, 0, 'quatrevingtpercent widthcentpercentminusx', 0, 0, '', '', '', -1, true);
					print "</td></tr>";
				}
