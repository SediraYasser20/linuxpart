...
//my edit to print lot
if ($action == 'buildlotpdf') {
    if (!$user->rights->produit->lire) {
        accessforbidden();
    }

    require_once DOL_DOCUMENT_ROOT.'/custom/lot/pdf/pdf_lotbarcode.php';

    $lot = new Productlot($db);
    $res = $lot->fetch(GETPOST('id', 'int'));

    if ($res <= 0) {
        setEventMessages("Lot introuvable ou erreur lors du fetch", null, 'errors');
    } else {
        try {
            $langs->load("products");
            $pdfmodel = new pdf_lotbarcode($db);

            if (!method_exists($pdfmodel, 'write_file')) {
                throw new Exception("Méthode write_file() manquante dans la classe pdf_lotbarcode");
            }

            $result = $pdfmodel->write_file($lot, $langs);

            if (isset($result['error'])) {
                throw new Exception("Erreur PDF: " . $result['error']);
            }

            if (!file_exists($result['filepath'])) {
                throw new Exception("Le fichier PDF n'a pas été créé : " . $result['filepath']);
            }

            // عرض الملف مباشرة
            header("Location: " . DOL_URL_ROOT . "/document.php?modulepart=product&file=lot/lot_current.pdf");
            exit;

        } catch (Exception $e) {
            setEventMessages("Erreur PDF : " . $e->getMessage(), null, 'errors');
        }
    }
}
...
print dolGetButtonAction('Imprimer lot', '', 'default', $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=buildlotpdf');
	}
}
