	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
{
    ...
    if ($showaddress) {
        ...
        $recipient_frame_top_y = getDolGlobalInt('MAIN_PDF_USE_ISO_LOCATION') ? 40 : 42;
        $recipient_frame_top_y += $top_shift;
        ...
        if (!getDolGlobalString('MAIN_PDF_NO_RECIPENT_FRAME')) {
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFont('', '', $default_font_size - 2);
            $pdf->SetXY($posx + 2, $recipient_frame_top_y - 5);
            $pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("BillTo"), 0, $ltrdirection);
        }

        // Y position where content inside the box starts
        $y_content_start = $recipient_frame_top_y + 3;

        // Show recipient name
        $pdf->SetXY($posx + 2, $y_content_start);
        ...
        // BEGIN: ADDED CLIENT DETAILS
        // Add Client Phone
        if (!empty($object->thirdparty->phone)) {
            $current_y_tracker += 1; // Add a small top margin
            $pdf->SetXY($posx + 2, $current_y_tracker);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("PhonePro").": ".$outputlangs->convToOutputCharset($object->thirdparty->phone), 0, $ltrdirection);
            $current_y_tracker = $pdf->GetY(); 
        }
        ...
        if (!empty($object->thirdparty->email)) {
            $current_y_tracker += 1; // Add a small top margin
            $pdf->SetXY($posx + 2, $current_y_tracker);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("Email").": ".$outputlangs->convToOutputCharset($object->thirdparty->email), 0, $ltrdirection);
            $current_y_tracker = $pdf->GetY();
        }
        ...
        if (!empty($object->thirdparty->tva_intra)) {
            $current_y_tracker += 1; // Add a small top margin
            $pdf->SetXY($posx + 2, $current_y_tracker);
            $pdf->MultiCell($widthrecbox - 2, 3, $outputlangs->transnoentities("VATIntra").": ".$outputlangs->convToOutputCharset($object->thirdparty->tva_intra), 0, $ltrdirection);
            $current_y_tracker = $pdf->GetY();
        }
        // END: ADDED CLIENT DETAILS

        $y_content_end = $current_y_tracker;

        // Calculate dynamic height for the recipient box frame
        if (!getDolGlobalString('MAIN_PDF_NO_RECIPENT_FRAME')) {
            $content_actual_height = $y_content_end - $y_content_start;
            $padding_inside_box = 6; // e.g., 3 for top padding from frame to text, 3 for bottom padding from text to frame.
            $dynamic_hautcadre = $content_actual_height + $padding_inside_box;
            // Ensure minimum height if content is very short or empty (though unlikely here)
            if ($dynamic_hautcadre < 20) { $dynamic_hautcadre = 20; } 

            $pdf->RoundedRect($posx, $recipient_frame_top_y, $widthrecbox, $dynamic_hautcadre, $this->corner_radius, '1234', 'D');
        }
        
        $posy = $recipient_frame_top_y; // Reset $posy to the top of the frame for shipping address calculation logic if it depends on it.
                                     // The original code used $posy += $hautcadre for shipping.
                                     // This needs to be $posy = $recipient_frame_top_y + $dynamic_hautcadre if shipping starts after this box.
                                     // Or, better, use $y_content_end for further calculations if shipping starts after.
                                     // The original shipping logic uses its own $hautcadre.
                                     // Let's ensure $posy for shipping logic starts correctly. The old code did $posy += $hautcadre (static).
                                     // Now it should be relative to the new dynamic height.
                                     // $shipp_shift is added to $posy at the end, so $posy should be the Y *before* shipping.
                                     // The original $posy for shipping was $posy_frame_start + $original_static_hautcadre (40 or 55)
                                     // So now it should be $recipient_frame_top_y + $dynamic_hautcadre
                                     // $posy will be used by shipping if it's enabled.
                                     // The current $posy is $pdf->GetY() after printing client name.
                                     // The old $posy for shipping was based on $posy (frame_top) + $hautcadre (static)
                                     // So, we need to update $posy to reflect the bottom of the recipient box before shipping logic.
        $posy = $recipient_frame_top_y + $dynamic_hautcadre;
