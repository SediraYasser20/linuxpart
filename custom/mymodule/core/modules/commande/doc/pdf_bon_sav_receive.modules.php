...
public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
{
    ...
    if (getMultidirOutput($object)) {
        ...
        if ($object->specimen) {
            ...
            $file = $dir . "/SPECIMEN_SAV_RECEPTION_therefoftheorder.pdf"; // New specimen filename
        } else {
            // $objectref = dol_sanitizeFileName($object->ref); // objectref is not needed for fixed filename
            $dir = getMultidirOutput($object); // Get the directory based on the object
            // Ensure the directory for the object exists, not necessarily a subdirectory with objectref
            if ($object->ref) {
                $objectref_dir = dol_sanitizeFileName($object->ref);
                $dir_with_ref = $dir . "/" . $objectref_dir;
                if (!file_exists($dir_with_ref)) {
                    dol_mkdir($dir_with_ref); // Create the specific object ref folder if it doesn't exist
                }
                $dir = $dir_with_ref; // Use the object specific folder
            }
            $file = $dir . "/SAV_RECEPTION_therefoftheorder.pdf"; // New filename
        }
...
$tab_top = max($this->marge_haute + 90, $pagehead['top_shift'] + $pagehead['shipp_shift']) + 30; // ajouter 30 mm d'espace
            ...
            for ($i = 0; $i < $nblines; $i++) {
                ...
                if ($this->getColumnStatus('desc')) {
                    // Ensure product_label is populated if not already on the invoice line object (done at the start of the loop for $i)
                    ...
                    // Get padding and alignment from $this->cols defined in defineColumnField
                    // Padding should be [1, 0.5, 1, 0] (top, right, bottom, left)
                    // Align should be 'L'
                    $padding = $this->cols[$colKey]['content']['padding']; 
                    $align = $this->cols[$colKey]['content']['align'];     
                    
                    // Force x_start to page left margin for the content cell
                    $x_start_forced = $this->marge_gauche; 
                    
                    // Use the defined width of the 'desc' column from defineColumnField
                    $col_width = $this->cols[$colKey]['width']; 
                    
                    $pdf->SetXY($x_start_forced + $padding[3], $curY + $padding[0]); // $padding[3] is left padding (0)
                    $pdf->MultiCell(
                        $col_width - $padding[1] - $padding[3], // cell width = column_width - R_padding - L_padding
                        $line_height_main, 
                        $outputlangs->convToOutputCharset($desc_content), 
                        0, // border
                        $align, // 'L'
                        0, // fill
                        1, // ln (move to next line after)
                        '', '', true, 0, false, true, 
                        $line_height_main, // maxh
                        'M' // valign
                    );
                    // Update Y position based on the MultiCell's behavior (ln=1 moves Y automatically)
...
$posy += 15;
    ...
    if ($showaddress) {
        ...
        if ($usecontact && $object->contact->socid != $object->thirdparty->id &&
            ...
            $thirdparty_client = $object->contact; // Client is $object->contact if different from $object->thirdparty
        } else {
            $thirdparty_client = $object->thirdparty; // Client is $object->thirdparty
        }

        if (is_object($thirdparty_client)) {
            $carac_client_name = pdfBuildThirdpartyName($thirdparty_client, $outputlangs);
        }

        $mode_client = 'target'; // 'target' is usually for recipient
        $carac_client = pdf_build_address($outputlangs, $this->emetteur, $thirdparty_client,
                                         ($usecontact ? $object->contact : ''),
                                         ($usecontact ? 1 : 0), $mode_client, $object);

        // Define box sizes (can be reused or adjusted)
        $hautcadre = getDolGlobalInt('MAIN_PDF_USE_ISO_LOCATION') ? 38 : 40;
        $width_client_box = getDolGlobalInt('MAIN_PDF_USE_ISO_LOCATION') ? 92 : 100;
        if ($this->page_largeur < 210) {
            $width_client_box = 84; // To work with US executive format for client
        }
        $width_emetteur_box = getDolGlobalInt('MAIN_PDF_USE_ISO_LOCATION') ? 92 : 82;


        // Determine positions based on MAIN_INVERT_SENDER_RECIPIENT
        // Default: Client on Left, Our Company on Right
        // If MAIN_INVERT_SENDER_RECIPIENT is true: Our Company on Left, Client on Right

        $posx_client = $this->marge_gauche;
        $posx_emetteur = $this->page_largeur - $this->marge_droite - $width_emetteur_box;

        if (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT')) {
            // Inverted: Our Company on Left, Client on Right
            $posx_emetteur = $this->marge_gauche;
            $posx_client = $this->page_largeur - $this->marge_droite - $width_client_box;
        }

        // Show Client (formerly Recipient) - NOW ON LEFT (or right if inverted)
        $current_posy_client = $address_start_y;

        // Show client frame
        if (!getDolGlobalString('MAIN_PDF_NO_RECIPENT_FRAME')) { // Use recipient frame for client
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFont('', '', $default_font_size - 2);
            $pdf->SetXY($posx_client + (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 2:0) , $current_posy_client - 5); // Small adjustment for inverted text alignment
            $pdf->MultiCell($width_client_box, 5, "From Client" /*$outputlangs->transnoentities("BillTo")*/, 0, $ltrdirection);
            $pdf->RoundedRect($posx_client, $current_posy_client, $width_client_box, $hautcadre, $this->corner_radius, '1234', 'D');
        }

        // Show client name
        $pdf->SetXY($posx_client + 2, $current_posy_client + 3);
        $pdf->SetFont('', 'B', $default_font_size);
        $pdf->MultiCell($width_client_box - (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 0:2) , 2, $carac_client_name, 0, $ltrdirection);
        $current_posy_client = $pdf->getY();

        // Show client information
        $pdf->SetFont('', '', $default_font_size - 1);
        $pdf->SetXY($posx_client + 2, $current_posy_client);
        $pdf->MultiCell($width_client_box - (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 0:2), 4, $carac_client, 0, $ltrdirection);


        // Show Our Company (formerly Sender) - NOW ON RIGHT (or left if inverted)
        $current_posy_emetteur = $address_start_y;

        // Show sender frame
        if (!getDolGlobalString('MAIN_PDF_NO_SENDER_FRAME')) {
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFont('', '', $default_font_size - 2);
            $pdf->SetXY($posx_emetteur + (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 0:2), $current_posy_emetteur - 5); // Small adjustment for inverted text alignment
            $pdf->MultiCell($width_emetteur_box, 5, "To Company" /*$outputlangs->transnoentities("BillFrom")*/, 0, $ltrdirection);
            $pdf->SetXY($posx_emetteur, $current_posy_emetteur);
            $pdf->SetFillColor(230, 230, 230);
            $pdf->RoundedRect($posx_emetteur, $current_posy_emetteur, $width_emetteur_box, $hautcadre, $this->corner_radius, '1234', 'F');
            $pdf->SetTextColor(0, 0, 60);
        }

        // Show sender name
        if (!getDolGlobalString('MAIN_PDF_NO_SENDER_NAME')) {
            $pdf->SetXY($posx_emetteur + 2, $current_posy_emetteur + 3);
            $pdf->SetFont('', 'B', $default_font_size);
            $pdf->MultiCell($width_emetteur_box - (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 2:0), 4, $outputlangs->convToOutputCharset($this->emetteur->name), 0, $ltrdirection);
            $current_posy_emetteur = $pdf->getY();
        }

        // Show sender information
        $pdf->SetXY($posx_emetteur + 2, $current_posy_emetteur);
        $pdf->SetFont('', '', $default_font_size - 1);
        $pdf->MultiCell($width_emetteur_box - (getDolGlobalInt('MAIN_INVERT_SENDER_RECIPIENT') ? 2:0), 4, $carac_emetteur, 0, $ltrdirection);


        // Show shipping address - only if enabled and needed
        // This should be positioned relative to the CLIENT block (which is now $posx_client)
        $shipp_shift = 0;
        $posy_shipping_start = max($current_posy_client, $current_posy_emetteur) + $hautcadre + 10; // Start after the tallest of the two blocks
        ...
        if (getDolGlobalInt('SALES_ORDER_SHOW_SHIPPING_ADDRESS')) {
            ...
            if (!empty($idaddressshipping)) {
                // A specific shipping contact is defined
                $object->fetch_contact($idaddressshipping[0]); // $object->contact is now the shipping contact
                $companyshippingstatic = new Societe($this->db); // Need to fetch the company of this contact
                $companyshippingstatic->fetch($object->contact->fk_soc);
                $carac_client_name_shipping = pdfBuildThirdpartyName($object->contact, $outputlangs); // Name of the contact for shipping
                $carac_client_shipping = pdf_build_address($outputlangs, $this->emetteur, $companyshippingstatic, $object->contact, 1, 'target', $object);
            } else {
                // No specific shipping contact, use thirdparty of the order (client) for shipping
                $carac_client_name_shipping = pdfBuildThirdpartyName($thirdparty_client, $outputlangs); // Name of the client
                $carac_client_shipping = pdf_build_address($outputlangs, $this->emetteur, $thirdparty_client, '', 0, 'target', $object);
            }

            if (!empty($carac_client_shipping)) {
                // $posy_shipping_current = $current_posy_client + $hautcadre + 10; // Position below the client block
                $posy_shipping_current = $posy_shipping_start;


                // Adjust $posx_shipping based on MAIN_INVERT_SENDER_RECIPIENT to follow the client block
                $posx_shipping = $posx_client; // Shipping address follows client block
                $width_shipping_box = $width_client_box; // Shipping box same width as client box

                $hautcadre_shipping = $hautcadre - 10; // Height for the shipping address is smaller

                // Show shipping frame
                // The X position of the shipping address should be aligned with the client's address block
                $pdf->SetXY($posx_shipping + 2, $posy_shipping_current - 5);
                $pdf->SetFont('', '', $default_font_size - 2);
                $pdf->MultiCell($width_shipping_box, '', $outputlangs->transnoentities('ShippingTo'), 0, $ltrdirection, 0);
                $pdf->RoundedRect($posx_shipping, $posy_shipping_current, $width_shipping_box, $hautcadre_shipping, $this->corner_radius, '1234', 'D');

                // Show shipping name
                $pdf->SetXY($posx_shipping + 2, $posy_shipping_current + 1);
                $pdf->SetFont('', 'B', $default_font_size);
                $pdf->MultiCell($width_shipping_box - 2, 2, $carac_client_name_shipping, '', $ltrdirection);
                $posy_shipping_current = $pdf->getY();

                // Show shipping information
                $pdf->SetXY($posx_shipping + 2, $posy_shipping_current);
                $pdf->SetFont('', '', $default_font_size - 1);
                $pdf->MultiCell($width_shipping_box - 2, 2, $carac_client_shipping, '', $ltrdirection);

                $shipp_shift = $hautcadre_shipping + 10; // This is a height shift, not a Y position.
            }
        }
    }
    ...
    // top_shift is related to linked objects, shipp_shift is the height of the shipping box if displayed
...
$this->cols['desc'] = array(
    ...
    'content' => array(
        ...
        'padding' => array(1, 0.5, 1, 0), // Set left padding to 0
    ),
);
